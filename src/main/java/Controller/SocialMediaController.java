...
public class SocialMediaController {
    ...
    // localhost:8080/register. The body will contain a representation of a JSON Account, 
    // but will not contain an account_id.
    // The registration will be successful if and only if the username is not blank, the 
    // password is at least 4 characters long, and an Account with that username does not 
    // already exist. If all these conditions are met, the response body should contain a 
    // JSON of the Account, including its account_id. The response status should be 200 OK,
    //     which is the default. The new account should be persisted to the database.
    // If the registration is not successful, the response status should be 400. (Client 
    // error)
    ...
    // localhost:8080/login. The request body will contain a JSON representation of an Account,
    //  not containing an account_id. In the future, this action may generate a Session token 
    //  to allow the user to securely use the site. We will not worry about this for now.
    ...
    private void postLoginHandler(Context ctx) throws JsonProcessingException
    {
        ...
        Account result = accountService.login(loginAttempt.getUsername(), loginAttempt.getPassword());
        if (result != null){
            ctx.json(result);
        } 
        else{
    ...
    // localhost:8080/messages. The request body will contain a JSON representation of a 
    // message, which should be persisted to the database, but will not contain a message_id.
    // The creation of the message will be successful if and only if the message_text is not
    //  blank, is under 255 characters, and posted_by refers to a real, existing user. If 
    //  successful, the response body should contain a JSON of the message, including its 
    //  message_id. The response status should be 200, which is the default. The new message 
    ...
    // messages retrieved from the database. It is expected for the list to simply be empty 
    ...
    // The response body should contain a JSON representation of the message identified by 
    // the message_id. It is expected for the response body to simply be empty if there is no
    // such message. The response status should always be 200, which is the default.
    ...
    //  If the message existed, the response body should contain the now-deleted message. The 
    //  response status should be 200, which is the default.
    // If the message did not exist, the response status should be 200, but the response body 
    // should be empty. This is because the DELETE verb is intended to be idempotent, ie, 
    ...
    // The response body should contain a JSON representation of a list containing all 
    // messages posted by a particular user, which is retrieved from the database. It is 
    // expected for the list to simply be empty if there are no messages. The response status 
